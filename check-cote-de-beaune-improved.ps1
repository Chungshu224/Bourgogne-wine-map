# 改進版檢查腳本 - 檢查 Côte-de-Beaune GeoJSON 和資料匹配
...
foreach ($item in $totalReport) {
    ...
    if ($item.Missing -gt 0 -and $item.MissingFiles.Count -le 20) {
        Write-Host "  - 缺少資料的 GeoJSON 文件:"
        foreach ($file in $item.MissingFiles) {
            Write-Host "    * $file" -ForegroundColor Red
        }
    } elseif ($item.Missing -gt 20) {
        Write-Host "  - 缺少資料的 GeoJSON 文件: (僅顯示前20個)"
        for ($i = 0; $i -lt 20; $i++) {
            Write-Host "    * $($item.MissingFiles[$i])" -ForegroundColor Red
        }
        Write-Host "    ... 還有 $($item.Missing - 20) 個文件" -ForegroundColor Red
    }
    Write-Host ""
}
